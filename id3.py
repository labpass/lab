...
def gain(en_s,data_s,attrib):
    ...
    gain = en_s
    for value in values:
        gain -= len(data_s.loc[data_s[attrib]==value])/(len(data_s))*entropy(data_s.loc[data_s[attrib]==value])
    return gain
def get_attr(data):
    en_s = entropy(data)
    attribute = ""
...
def decision_tree(data):
    ...
    if(entropy(data)==0):
        if(len(data.loc[data[data.columns[-1]] 
        =="Yes"]) == len(data)):
            root.label = "Yes"
        else:
            root.label = "No"
...
data = pd.read_csv("C:\Shivam\coding\python\etennis.csv")
